...
namespace Nethereum.RPC.TransactionManagers
{
	public abstract class TransactionManagerBase : ITransactionManager
	{
		...
		{
			return !UseLegacyAsDefault && transaction.GasPrice == null 
	...
	#if !DOTNET35
		public BigInteger DefaultMaxPriorityFeePerGas { get; set; } =
			SimpleFeeSuggestionStrategy.DEFAULT_MAX_PRIORITY_FEE_PER_GAS;
		...
		{
			get =>
				_transactionReceiptService ??
				TransactionReceiptServiceFactory.GetDefaultransactionReceiptService(this);
			set => _transactionReceiptService = value;
		}

		public Task<TransactionReceipt> SendTransactionAndWaitForReceiptAsync(TransactionInput transactionInput,
		...
		protected void SetDefaultGasPriceAndCostIfNotSet(TransactionInput transactionInput)
		{
			...
			{
				transactionInput.Gas = new HexBigInteger(DefaultGas);
			}
		}
